...
Each serializer has two variants:
  *ListSerializer  — lightweight, for list endpoints (fewer nested fields)
  *Serializer      — full detail, for retrieve endpoints
"""
...
class LocationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Location
        fields = [
            "id", "name", "location_type", "institution",
            "description", "parent", "full_path",
            "institution_name", "children_count",
        ]
    ...
    def get_institution_name(self, obj):
        return str(obj.institution) if obj.institution else None
...
class PropertyValueSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PropertyValue
        fields = [
            "id", "property_type", "property_type_name", "property_type_handler",
            "tag", "value", "units", "description",
            "is_dynamic", "user_writable",
...
class LogEntrySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = LogEntry
        fields = [
            "id", "timestamp", "topic", "entry", "attachment",
...
class ComponentFunctionSerializer(serializers.ModelSerializer):
    technical_system_name = serializers.CharField(
        source="technical_system.name", read_only=True)

    class Meta:
        model = ComponentFunction
        fields = ["id", "name", "technical_system", "technical_system_name", "description"]
...
class ComponentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Component
        fields = [
            "id", "name", "alternate_name", "model_number",
            "description", "project",
            "technical_system", "technical_system_name",
            "function", "function_name",
            ...
            "group_writeable",
            "created_on", "modified_on",
            "sources", "properties", "log_entries",
            "instance_count",
        ]
...
class ComponentInstanceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ComponentInstance
        fields = [
            "id", "qr_id", "tag", "serial_number", "description",
            "component", "component_name",
...
class DesignElementSerializer(serializers.ModelSerializer):
    ...
    installed_instance_qr = serializers.CharField(
        source="installed_instance.qr_id", read_only=True)
    properties = PropertyValueSerializer(many=True, read_only=True)

    class Meta:
        model = DesignElement
        fields = [
            "id", "element_name", "element_type", "quantity", "description",
            "component", "component_name",
